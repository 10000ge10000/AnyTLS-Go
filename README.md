...
| 协议 | 核心机制 | 传输层 | 速度/延迟 | 抗封锁能力 | 资源开销 | 客户端生态 | 推荐场景 | 缺点/备注 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **VLESS (Reality)** | TLS 偷取/伪装 | TCP (XTLS) | 🟢 快 / 低 | ⭐⭐⭐⭐⭐ | 🟡 中 | 🟢 极好 | **首选主力**，长期稳定 | 需占用 443 端口，依赖域名伪装 |
| **Hysteria 2** | UDP 拥塞控制 | UDP | 🚀 极快 / 极低 | ⭐⭐⭐ | 🔴 高 | 🟢 好 | **视频/弱网**，垃圾线路救星 | 暴力发包，易被运营商 QOS 限速 |
| **Tuic v5** | QUIC (0-RTT) | UDP (QUIC) | 🟢 快 / 极低 | ⭐⭐⭐ | 🔴 高 | 🟡 一般 | **游戏/低延迟**，移动漫游 | 网络切换不断流，但依赖 UDP 环境 |
| **AnyTLS** | 全拟态 HTTPS | TCP | 🟡 中 / 中 | ⭐⭐⭐⭐⭐ | 🟡 中 | 🔴 少 | **备用/救急**，高封锁区 | 模拟完整浏览器指纹，隐蔽性极强 |
| **Sudoku** | ASCII 隐写术 | TCP | 🟡 中 / 中 | ⭐⭐⭐⭐⭐ | 🔴 高 | 🔴 稀缺 | **防 DPI 检测**，白名单环境 | 流量伪装成文本，Payload 有膨胀损耗 |
| **Mieru** | 强混淆/抗重放 | TCP / UDP | 🟡 中 / 中 | ⭐⭐⭐⭐ | 🟡 中 | 🔴 少 | **对抗主动探测** | 独特的协议设计，不易识别 |
| **SS-2022** | AEAD 纯加密 | TCP / UDP | 🚀 极快 / 低 | ⭐⭐ | 🟢 极低 | 🟢 极好 | **中转/内网**，低性能路由 | 无伪装特征，直连容易被识别封锁 |

> **图例说明**：
> *   🚀 / 🟢 / 🟡 / 🔴：代表性能从优到差的层级。
> *   **资源开销**：指服务端 CPU/内存占用以及协议本身的计算开销。
> *   **客户端生态**：指支持该协议的客户端数量及跨平台支持程度（如 iOS/Android/Windows）。
...
详细使用指南请参阅：[Alice 家宽分流使用指南](docs/socks_route_guide.md)
